"""Cassette file format (.rw) for pytest-rewind.
...
`rewind import cassette.rw` loads it into the local DB + blob store.
"""
...
def import_cassette(data: dict[str, Any], db: RewindDB, blobs: BlobStore) -> str:
    ...
    # Write blobs first — write() is idempotent; same content → same hash
